...
void MetricsCollector::collectTransferMetrics() {
  try {
    ...
    std::string transferQuery =
        ...
        "CASE "
        "  WHEN COALESCE(t.current_records, 0) >= COALESCE(c.last_offset, 0) THEN "
        "    COALESCE(t.current_records, 0) - COALESCE(c.last_offset, 0) "
        "  ELSE "
        "    COALESCE(t.current_records, 0) "
        "END as records_transferred,"
        "c.last_sync_time,"
        "CASE "
        "  WHEN c.last_sync_time IS NOT NULL THEN "
        "    EXTRACT(EPOCH FROM (NOW() - c.last_sync_time)) * 1000 "
        "  ELSE "
        "    0 "
...
void MetricsCollector::collectLatencyMetrics() {
  try {
    ...
    // Recolectar 100 muestras de latencia
    ...
    for (int i = 0; i < 100; i++) {
      ...
      latencySamples.push_back(latency_ms);
      
      // Pequeña pausa entre muestras para no sobrecargar
