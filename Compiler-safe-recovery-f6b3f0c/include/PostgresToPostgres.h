  ...
  void setupTableTargetPostgresToPostgres() {
    try {
      ...
      for (const auto &row : results) {
        ...
        try {
          auto pooledConn = connectPostgres(sourceConnStr);
          if (!pooledConn) {
            Logger::error("setupTableTargetPostgresToPostgres",
                          "Failed to connect to source PostgreSQL");
            continue;
          }
          auto pgConn = std::static_pointer_cast<pqxx::connection>(pooledConn->connection);
          ...
          std::string createTableQuery = buildCreateTableQuery(
               pooledConn, schemaName, tableName, lowerSchemaName);
...
private:
  ...
  connectPostgres(const std::string &connStr) {
    if (!g_connectionPool) {
      Logger::error("connectPostgres", "Connection pool not initialized");
      return nullptr;
    }
    ...
    return g_connectionPool->getConnection(DatabaseType::POSTGRESQL);
  }
  ...
  std::string buildCreateTableQuery(std::shared_ptr<ConnectionPool::PooledConnection> sourceConn,
                                    const std::string &sourceSchema,
                                    const std::string &tableName,
                                    const std::string &targetSchema) {
    try {
      auto pgConn = std::static_pointer_cast<pqxx::connection>(sourceConn->connection);
      pqxx::work sourceTxn(*pgConn);
  ...
  std::string detectTimeColumn(std::shared_ptr<ConnectionPool::PooledConnection> pooledConn,
  ...
  int getSourceCount(std::shared_ptr<ConnectionPool::PooledConnection> pooledConn,
                     const std::string &schemaName,
                     const std::string &tableName,
  ...
  void performDataTransfer(pqxx::connection &pgConn,
                           std::shared_ptr<ConnectionPool::PooledConnection> sourceConn,
                           const std::string &schemaName,
                           const std::string &tableName,
                           const std::string &timeColumn,
    ...
    try {
      ...
      pqxx::connection statusConn(DatabaseConfig::getPostgresConnectionString());
      updateStatus(statusConn, schemaName, tableName, "PERFECT_MATCH", sourceCount);
      Logger::info("performDataTransfer",
      ...
      pqxx::connection statusConn(DatabaseConfig::getPostgresConnectionString());
      updateStatus(statusConn, schemaName, tableName, "ERROR", 0);
    }
  }
