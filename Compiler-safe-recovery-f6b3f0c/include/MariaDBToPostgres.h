...
public:
  ...
  void syncIndexesAndConstraints(const std::string &schema_name,
                                 ...
                                 std::shared_ptr<ConnectionPool::PooledConnection> mariadbConn,
  ...
  void setupTableTargetMariaDBToPostgres() {
    try {
      ...
      for (const auto &table : tables) {
        ...
        auto pooledConn = connectMariaDB(table.connection_string);
        if (!pooledConn) {
        ...
        auto columns = executeQueryMariaDB(pooledConn, query);
    ...
    try {
      ...
      for (auto &table : tables) {
        ...
        if (sourceCount == targetCount) {
          ...
          if (!table.last_sync_column.empty()) {
            ...
            try {
              ...
              auto mariaMaxRes =
                  executeQueryMariaDB(pooledConn, mariaMaxQuery);
        ...
        while (hasMoreData) {
          ...
          auto results = executeQueryMariaDB(pooledConn, selectQuery);
...
private:
  ...
  connectMariaDB(const std::string &connStr) {
    if (!g_connectionPool) {
      Logger::error("connectMariaDB", "Connection pool not initialized");
      return nullptr;
    }

    // Add config only if it doesn't exist
    bool configExists = false;
    for (const auto &config : g_connectionPool->getConfigs()) {
      if (config.type == DatabaseType::MARIADB && config.connectionString == connStr) {
        configExists = true;
        break;
      }
    }
    ...
    if (!configExists) {
      Logger::debug("connectMariaDB", "Adding new MariaDB config for connection string: " + connStr);
    ConnectionConfig config;
    config.type = DatabaseType::MARIADB;
    config.connectionString = connStr;
    config.minConnections = 3;  // Aumentamos el mínimo ya que tenemos varias bases de datos
    config.maxConnections = 15; // Y el máximo también
      g_connectionPool->addDatabaseConfig(config);
    }

    return g_connectionPool->getConnection(DatabaseType::MARIADB);
  }

  std::vector<std::vector<std::string>>
  executeQueryMariaDB(std::shared_ptr<ConnectionPool::PooledConnection> conn, const std::string &query) {
    ...
    if (mysql_query(mysql.get(), query.c_str())) {
      Logger::error("executeQueryMariaDB", "Query execution failed: " +
                                               std::string(mysql_error(mysql.get())));
      return results;
    }

    MYSQL_RES *res = mysql_store_result(mysql.get());
    if (!res) {
      if (mysql_field_count(mysql.get()) > 0) {
