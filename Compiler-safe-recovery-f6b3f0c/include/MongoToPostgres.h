  ...
  void setupTableTargetMongoToPostgres() {
    try {
      ...
      for (const auto &row : results) {
        ...
        try {
          auto pooledConn = connectMongoDB(mongoConnStr);
          if (!pooledConn) {
            Logger::error("setupTableTargetMongoToPostgres",
                          "Failed to connect to MongoDB");
            continue;
          }
          auto client = std::static_pointer_cast<mongoc_client_t>(pooledConn->connection);
...
private:
  ...
  connectMongoDB(const std::string &connStr) {
    if (!g_connectionPool) {
      Logger::error("connectMongoDB", "Connection pool not initialized");
      return nullptr;
    }
    ...
    return g_connectionPool->getConnection(DatabaseType::MONGODB);
  }
  ...
  std::string buildCreateTableQuery(std::shared_ptr<ConnectionPool::PooledConnection> mongoConn,
                                    const std::string &dbName,
                                    const std::string &collectionName,
                                    const std::string &targetSchema) {
    try {
      auto client = std::static_pointer_cast<mongoc_client_t>(mongoConn->connection);
      mongoc_collection_t *collection = mongoc_client_get_collection(
          client.get(), dbName.c_str(), collectionName.c_str());
  ...
  int getSourceCount(std::shared_ptr<ConnectionPool::PooledConnection> pooledConn, const std::string &dbName,
                     const std::string &collectionName) {
    try {
      mongoc_collection_t *collection = mongoc_client_get_collection(
           std::static_pointer_cast<mongoc_client_t>(pooledConn->connection).get(), dbName.c_str(), collectionName.c_str());
  ...
  void performDataTransfer(pqxx::connection &pgConn,
                           std::shared_ptr<ConnectionPool::PooledConnection> mongoConn,
                           const std::string &dbName,
